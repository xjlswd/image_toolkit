...
def draw_rectangles_with_labels(
    ...
    """
    # 确保输入的坐标列表和标签列表是有效的
    if not isinstance(coordinates_list, (list, tuple)) or not isinstance(
        labels, (list, tuple)
    ):
    ...
    for coordinates, label in zip(coordinates_list, labels):
        ...
        # 确保坐标在0到1000之间
        if not (
            0 <= x1 <= 1000 and 0 <= y1 <= 1000 and 0 <= x2 <= 1000 and 0 <= y2 <= 1000
        ):
        ...
        for pos in positions:
            if (
                0 <= pos[0] <= width - font_width
                and 0 <= pos[1] <= height - font_height
            ):
